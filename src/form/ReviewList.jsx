...
function ReviewList() {
    ...
    const deleteOneRecord = async (id)=>{
      try{
        await TableService.delOneReview(id);
        setXiaofang(x=>!x);
        openNotificationWithIcon("success","删除复习记录成功");
      }catch(ex){
  ...
  useEffect( ()=>{
    const zpddyz = async ()=> {
        try{
          setIsLoading(true);
          const res = await TableService.getAllReview(subject);
  ...
  return (
    ...
    <div style={{background:'white'}}>
          ...
          <div style={{display:'flex',justifyContent:'center'}}>
            <Button style={{width:'8rem',marginTop:'1rem',marginBottom:'2rem'}}  type="primary" onClick={()=>navigate('/nav/review/input')}>
              我要追加
            </Button>
          </div>
    </div>
